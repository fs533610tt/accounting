...
- 💰 **自動化帳務收費 (Billing & Accounting)**
  ...
  - **多名目收費明細**：同一筆帳單可同時記錄月費、會費、交通費、幹部折抵等任意正負項目，總額由資料庫自動加總。
  - **個別微調與對帳**：支援幹部折扣、請假退費的明細修改，並具備「一鍵繳清」功能，對帳清楚明瞭。
...
- 執行完整合腳本後，請接著執行 `docs/sql/temp/27_billing_record_items.sql`，建立多名目收費明細表與 RLS；此 migration 會保留既有帳單總額。
